      ...
      (config) => {
        ...
        return withDangerousMod(config, [
          ...
          (c) => {
            ...
            const xenvContent = [
              '# Auto-generated by expo prebuild — do not edit.',
              '# Sourced after .xcode.env by the Xcode bundle build phase.',
              '#',
              '# 1. Initialize nvm so NODE_BINARY resolves to the correct node.',
              '#    .xcode.env runs $(command -v node) before nvm loads, giving an',
              '#    empty NODE_BINARY.  Sourcing nvm here and re-setting NODE_BINARY',
              '#    fixes the "line 9: : command not found" Archive build failure.',
            ...
            console.log(`✅ Wrote ios/.xcode.env.local → NODE_BINARY + TENANT=${TENANT}`);
            return c;
          },
        ]);
      },
