...
# --- Step 4c: Reset tenantBuild.generated.js to Quadley before pushing -------
# This guarantees the repo always holds Quadley's identity.
# White-label builds (Grace College, etc.) may overwrite this file locally
# during expo prebuild, but those changes must never land in the committed repo
# — each tenant's canonical values live in their own static tenantBuild.*.js file.
TENANT_GENERATED = "mobile/src/config/tenantBuild.generated.js"
QUADLEY_CANONICAL = """// AUTO-GENERATED by app.config.js — DO NOT EDIT MANUALLY
...
"""
try:
    with open(TENANT_GENERATED, "r") as fh:
        current = fh.read()
    if '"grace_college"' in current or '"ormond"' in current or '"murphy_shark"' in current:
        with open(TENANT_GENERATED, "w") as fh:
            fh.write(QUADLEY_CANONICAL)
        print(f"  ⚠️  tenantBuild.generated.js had non-Quadley tenant — reset to Quadley.")
    else:
        print(f"  tenantBuild.generated.js is already Quadley — no reset needed.")
except Exception as e:
    print(f"  Warning: could not reset tenantBuild.generated.js: {e}")
