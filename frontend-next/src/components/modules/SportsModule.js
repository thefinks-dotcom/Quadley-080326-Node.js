...
const SportsModule = () => {
  ...
  const fetchGroups = async () => {
    try {
      const response = await axios.get(`${API}/cocurricular/groups/sports`);
      setGroups(response.data);
    } catch (error) {
      console.error('Failed to fetch sports groups', error);
    }
  };
  ...
  const createGroup = async (e) => {
    ...
    try {
      ...
      toast.success('Sports team created!');
      setShowForm(false);
      setNewGroup({
        name: '',
        description: '',
  ...
  // Filter teams: joined vs available (not joined)
  const joinedTeams = groups.filter(g => g.members && g.members.includes(user?.id));
  const availableTeams = groups.filter(g => !g.members || !g.members.includes(user?.id));

  const renderTeamCard = (group, isJoinedSection = false) => {
    ...
    return (
      <Card key={group.id} className={`p-6 glass ${isJoinedSection ? 'border-2 border-border bg-muted/30' : ''}`}>
          ...
          <div className="grid grid-cols-2 gap-2 mb-4">
            {group.photos.map((photo, photoIdx) => (
              <div key={photoIdx} className="relative">
                <img src={photo} alt={`Team photo ${photoIdx + 1}`} className="w-full h-32 object-cover rounded-lg" />
                {canManage && (
                  <Button
                    size="sm"
                    variant="destructive"
        ...
        <div className="space-y-2 text-sm">
          <p><strong>Coach/Captain:</strong> {group.contact_person_name}</p>
          {group.meeting_times && <p><strong>Training Times:</strong> {group.meeting_times}</p>}
          {group.competition_times && <p><strong>Competition Schedule:</strong> {group.competition_times}</p>}
          {group.other_details && <p><strong>Details:</strong> {group.other_details}</p>}
          <p><strong>Team Members:</strong> {group.members ? group.members.length : 0}</p>
        </div>
          ...
          <div className="mt-4 pt-4 border-t">
            ...
            {showGroupChat === group.id && (
              <div className="mt-4 border rounded-lg p-4 bg-muted">
                <h4 className="font-semibold mb-3 text-sm">Team Chat</h4>
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="flex items-center justify-between">
        <h2 className="heading-font text-3xl font-bold">Sports & Athletics</h2>
        <Button
          onClick={() => setShowForm(!showForm)}
          className="bg-gradient-to-r from-primary to-secondary"
          data-testid="create-sports-group-btn"
        >
      ...
      {showForm && (
        <Card className="p-6 glass">
          <h3 className="font-semibold mb-4">Create New Sports Team</h3>
          <form onSubmit={createGroup} className="space-y-4">
            <div>
              <Label>Team Name</Label>
              <Input
                placeholder="e.g., Rowing, Rugby, Netball"
                value={newGroup.name}
            ...
            <div>
              ...
              <Textarea
                placeholder="Describe your sports team..."
            ...
            <div>
              ...
              <Input
                placeholder="e.g., Mon/Wed/Fri 7AM, Tuesdays 5PM"
            ...
            <div>
              ...
              <Input
                placeholder="e.g., Saturday matches, Weekend tournaments"
      ...
      {/* My Joined Teams Section */}
      {joinedTeams.length > 0 && (
        <div className="mb-8">
          <div className="flex items-center gap-2 mb-4">
            <h3 className="text-2xl font-bold text-primary">✓ My Joined Teams</h3>
            <Badge className="bg-primary text-white">{joinedTeams.length}</Badge>
          </div>
          <div className="grid gap-4">
            {joinedTeams.map((team) => renderTeamCard(team, true))}
          </div>
        </div>
      )}
      ...
      {availableTeams.length > 0 && (
        <div>
          <div className="flex items-center gap-2 mb-4">
            <h3 className="text-2xl font-bold">All Available Teams</h3>
            <Badge variant="outline">{availableTeams.length}</Badge>
          </div>
          <p className="text-sm text-muted-foreground mb-4">Browse and join any team that interests you</p>
          <div className="grid gap-4">
      ...
      {groups.length === 0 && (
        <div className="text-center py-12 text-muted-foreground">
          <p>No sports teams available yet. Be the first to create one!</p>
        </div>
      )}
      </div>
    </div>
